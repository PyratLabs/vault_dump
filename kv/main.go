...
func ListKvSecrets(client *vault.Client, mount string) []string {
	...
	core.Logger.Debug("results found with v1 list", "results", len(kvkeys.Data))
	if kverr != nil {
		core.Logger.Error("could not list paths", "mount", mount, "err", kverr)
	}

	if len(kvkeys.Data) < 1 {
		core.Logger.Debug("could not find v1 secret, trying v2", "kv2Path", kv2Path)
		kv2keys, kv2err := client.Logical().List(kv2Path)
		if kv2err != nil {
			core.Logger.Error("count not list paths", "mount", mount, "err", kv2err)
		}
		keys = kv2keys
		vPrefix = "v2"
	} else {
		core.Logger.Debug("could not find v2 secret, trying v1", "kvPath", kvPath)
		keys = kvkeys
		vPrefix = "v1"
	}
...
func DumpKvMount(client *vault.Client, mount string) core.MountDump {
	var output core.MountDump
	var outputPath core.PathDump
	var outputSecret core.KvSecretDump
	var secVer string
	var secPath string
	var sec2Path string
	var secList []string
	...
	for _, sec := range secList {
		...
		case "v1":
			...
			if readerr != nil {
				core.Logger.Error("cannot read v1 secret path", "path", secPath)
			}
		...
		default:
			...
			if readerr != nil {
				core.Logger.Error("cannot read v2 secret path", "path", sec2Path)
			}
...
func RestoreKvPath(client *vault.Client, mount string, path string, secrets []core.KvSecretDump) {
